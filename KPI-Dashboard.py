...
def lade_excel_von_sharepoint(url: str, sheet_name=0, **kwargs) -> pd.DataFrame:
    """Lädt eine Excel-Datei von einem SharePoint-Download-Link.

        Args:
            url (str): Direkter SharePoint-Download-Link zur Excel-Datei (endet auf ?download=1).
            sheet_name (int|str): Tabellenblattname oder -index (wie bei pd.read_excel).
            **kwargs: Weitere Argumente für pandas.read_excel (z.B. skiprows, usecols, etc.)

        Returns:
            pd.DataFrame: Geladene Excel-Daten als DataFrame.
        """
    try:
        df = pd.read_excel(url, sheet_name=sheet_name, **kwargs)
        return df
    except Exception as e:
        st.error(f"Fehler beim Laden der Excel-Datei von SharePoint:\n{e}")
